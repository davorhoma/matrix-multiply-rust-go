...
impl MatrixMut {
    ...
    #[inline]
    pub fn set(&mut self, r: usize, c: usize, v: i32) {
        println!("r * self.stride + c: {}", r * self.stride + c);
        self.data[r * self.stride + c] = v;
    }
}

pub fn split_view_mut<'a>(
    m: &'a mut MatrixViewMut<'a>,
) -> (
    MatrixViewMut<'a>,
    MatrixViewMut<'a>,
    MatrixViewMut<'a>,
    ...
    let (top, bottom) = m.data.split_at_mut(r2 * m.stride);

    let (tl, tr) = top.split_at_mut(c2);
    let (bl, br) = bottom.split_at_mut(c2);
...
pub fn split_new(m: &mut MatrixViewMut) -> (MatrixMut, MatrixMut, MatrixMut, MatrixMut) {
    let r2 = m.rows / 2;
    let c2 = m.cols / 2;
...
pub fn add(a: &MatrixViewMut, b: &MatrixViewMut, c: &mut MatrixViewMut) {
    for i in 0..a.rows {
        for j in 0..a.cols {
            print!("i: {}, j: {}\n", i, j);
...
pub fn add_mut(a: &MatrixViewMut, b: &MatrixViewMut, c: &mut MatrixMut) {
    for i in 0..a.rows {
        for j in 0..a.cols {
            c.set(i, j, a.get(i, j) + b.get(i, j));
        }
...
pub fn add_view(a: &MatrixView, b: &MatrixView) -> Matrix {
...
#[inline]
pub fn zero(m: &mut MatrixViewMut) {
...
#[inline]
pub fn add_views(a: &MatrixViewMut, b: &MatrixViewMut, c: &mut MatrixViewMut) {
    for i in 0..a.rows {
        for j in 0..a.cols {
            c.set(i, j, a.get(i, j) + b.get(i, j));
        }
...
pub fn view_to_matrix(v: MatrixView) -> Matrix {
    let mut m = Matrix::new(v.rows, v.cols);
    for i in 0..v.rows {
        for j in 0..v.cols {
            m.set(i, j, v.get(i, j));
        }
