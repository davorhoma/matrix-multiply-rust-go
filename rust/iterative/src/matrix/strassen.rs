...
pub fn strassen_parallel(a: MatrixView, b: MatrixView) -> Matrix {
    let n = a.rows;
    ...
    let mut m1 = Matrix::new(half, half);
    let mut m2 = Matrix::new(half, half);
    let mut m3 = Matrix::new(half, half);
    let mut m4 = Matrix::new(half, half);
    let mut m5 = Matrix::new(half, half);
    let mut m6 = Matrix::new(half, half);
    let mut m7 = Matrix::new(half, half);

    rayon::scope(|s| {
        s.spawn(|_| {
            m1 = strassen_parallel(
                add_vec(&a11.to_matrix(), &a22.to_matrix()).view(),
                add_vec(&b11.to_matrix(), &b22.to_matrix()).view(),
            )
        });
        s.spawn(|_| m2 = strassen_parallel(add_vec(&a21.to_matrix(), &a22.to_matrix()).view(), b11));
        s.spawn(|_| m3 = strassen_parallel(a11, sub_vec(&b12, &b22).view()));
        s.spawn(|_| m4 = strassen_parallel(a22, sub_vec(&b21, &b11).view()));
        s.spawn(|_| m5 = strassen_parallel(add_vec(&a11.to_matrix(), &a12.to_matrix()).view(), b22));
        s.spawn(|_| {
            m6 = strassen_parallel(
                sub_vec(&a21, &a11).view(),
                add_vec(&b11.to_matrix(), &b12.to_matrix()).view(),
            )
        });
        s.spawn(|_| {
            m7 = strassen_parallel(
                sub_vec(&a12, &a22).view(),
                add_vec(&b21.to_matrix(), &b22.to_matrix()).view(),
            )
        });
    });
    // after scope, all m1..m7 are ready

    // Combine into submatrices
    let c11 = add_vec(&sub_vec(&add_vec(&m1, &m4).view(), &m5.view()), &m7);
    let c12 = add_vec(&m3, &m5);
    let c21 = add_vec(&m2, &m4);
    let c22 = add_vec(&sub_vec(&add_vec(&m1, &m3).view(), &m2.view()), &m6);
...
// fn split_inputs(a: MatrixView, size: usize) -> (MatrixView, MatrixView, MatrixView, MatrixView) {
//     let a11 = a.sub_view(0, 0, size, size);
//     let a12 = a.sub_view(0, size, size, size);
//     let a21 = a.sub_view(size, 0, size, size);
//     let a22 = a.sub_view(size, size, size, size);
